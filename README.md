...
- PR 合并前做一次架构级别的扫描
- 准备开源前对齐 AgentScope 这类标杆项目的工程规范
...
| 维度 | 主要参考 | 关键检查项 |
|------|---------|-----------|
| 1. 代码风格 | python-refactoring 坏味道体系 | 类型注解覆盖率、命名、docstring、行宽、异常处理、导入顺序 |
| 2. 封装边界 | AgentScope 封装原则 | `_` 前缀穿透、`__all__`、猴子补丁、动态挂载 |
| 3. 高内聚低耦合 + 游离方法 | 高内聚 / LCOM | 游离函数、假类、utils 滥用、跨模块直操内部状态 |
| 4. 架构与设计模式 | AgentScope 模板方法 / 注册表 | 基类设计、扩展点配套、if-elif 分发链、异常体系分层 |
| 5. 测试质量 | 真实性原则 | mock 一切、flaky sleep 轮询、核心模块无单测 |
| 6. 依赖与配置 | 可选依赖 / pydantic-settings | 可选依赖分组、惰性导入、配置集中管理 |
